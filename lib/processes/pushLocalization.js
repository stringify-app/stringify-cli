...
module.exports = async function(config, project, localization, format, referenceVersion=null, force=false){
    ...
    try {
        ...
        if(!localFile.length){
            ...
            spinner.stop();
            Log.warning(`${logPrefix} Skipping push because of empty locale file at ${filePath}`);
            throw { skip: true };
        }
        ...
        try {
            inspectResult = await inspectStrings(config, project.id, localization.locale, format, localFile, referenceVersion);
        }
        catch(e){

            const errorIdentifier = e && e.response ? e.response.identifier : null;

            if(errorIdentifier === ErrorIdentifier.PARSE_ERROR){
                throw new ApiError(`File ${filePath} has the wrong format or is invalid (expected format: ${format})`, e)
            }
            else if(errorIdentifier === ErrorIdentifier.INVALID_FILE_FORMAT){
                throw new ApiError(`Specified format '${format}' is invalid`, e)
            }
            else {
                throw e;
            }
